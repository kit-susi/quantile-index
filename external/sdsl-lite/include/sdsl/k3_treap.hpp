...
*/
/*! \file k3_treap.hpp
    \brief k3_treap.hpp contains a compact k^2-treap.
    \author Simon Gog
*/
#ifndef INCLUDED_SDSL_K3_TREAP
#define INCLUDED_SDSL_K3_TREAP
...
class k3_treap
{
    ...
    public:
        ...
        using node_type = k3_treap_ns::node_type;
        using point_type = k3_treap_ns::point_type;
        using t_p = k3_treap_ns::t_p;
    ...
    private:
        ...
        uint8_t get_t(const t_tv& v)
        {
            ...
            auto max_it = std::max_element(std::begin(v), std::end(v), [&](const t_e &a, const t_e &b) {
    ...
    public:
        ...
        k3_treap(const k3_treap& tr)
        {
            *this = tr;
        }

        k3_treap(k3_treap&& tr)
        {
            *this = std::move(tr);
        }

        //! Move assignment operator
        k3_treap& operator=(k3_treap&& tr)
        {
            if (this != &tr) {
                m_t = tr.m_t;
        ...
        k3_treap(int_vector_buffer<>& buf_x,
                 int_vector_buffer<>& buf_y,
                 int_vector_buffer<>& buf_z,
                 int_vector_buffer<>& buf_w)
        {
            using namespace k3_treap_ns;
            typedef int_vector_buffer<>* t_buf_p;
            std::vector<t_buf_p> bufs = {&buf_x, &buf_y, &buf_z, &buf_w};
        ...
        template<typename t_x=uint64_t, typename t_y=uint64_t, typename t_z=uint64_t, typename t_w=uint64_t>
        std::vector<std::tuple<t_x, t_y, t_z, t_w>>
                read(std::vector<int_vector_buffer<>*>& bufs)
        {
            typedef std::vector<std::tuple<t_x, t_y, t_z, t_w>> t_tuple_vec;
        ...
        k3_treap(std::vector<std::tuple<t_x, t_y, t_z, t_w>>& v, std::string temp_file_prefix="")
        {
            if (v.size() > 0) {
        ...
        template<typename t_x, typename t_y, typename t_z, typename t_w>
        void construct(std::vector<std::tuple<t_x, t_y, t_z, t_w>>& v, std::string temp_file_prefix="")
        {
            using namespace k3_treap_ns;
            using t_e = std::tuple<t_x, t_y, t_z, t_w>;
            m_t = get_t(v);
            ...
            std::string val_file =  temp_file_prefix + "_k3_treap_"
                                    + id_part + ".sdsl";
...

            {
                ...
                for (uint64_t l=t, cc=0; l+1 > 0; --l) {
                    if (l > 0) {
                        ...
                        m_coord[l-1] = int_vector<>(3*last_level_nodes,0, bits::hi(precomp<t_k>::exp(l))+1);
                    }
                    level_nodes = 0;
                    cc = 0;
                    auto sp = std::begin(v);
                    for (auto ep = sp; ep != end;) {
                        ...
                        if (l > 0) {
                            m_coord[l-1][3*cc]   = precomp<t_k>::modexp(std::get<0>(*max_it), l);
                            m_coord[l-1][3*cc+1] = precomp<t_k>::modexp(std::get<1>(*max_it), l);
                            m_coord[l-1][3*cc+2] = precomp<t_k>::modexp(std::get<2>(*max_it), l);
                            ++cc;
                        }

                        val_buf.push_back(std::get<3>(*max_it));
                        *max_it = MM;
                        --ep;
                        std::swap(*max_it, *ep);
                        if (l > 0) {
                            ...
                            for (uint8_t i=0; i < t_k; ++i) {
                                ...
                                for (uint8_t j=0; j < t_k; ++j) {
                                    ...
                                    auto ___sp = __sp;
                                    for (uint8_t k=0; k < t_k; ++k) {
                                        auto ___ep = __ep;
                                        if (k+1 < t_k) {
                                            ___ep = std::partition(___sp, __ep, [&k,&l](const t_e& e) {
                                                return precomp<t_k>::divexp(std::get<2>(e),l-1)%t_k <= k;
                                            });
                                        }
                                        bool not_empty = ___ep > ___sp;
            ...
            load_from_file(bp, bp_file);
            {
                ...
                while (bp_idx > 0) {
                    --r_idx;
                    for (size_t i=0; i < t_k*t_k*t_k; ++i) {
                        if (bp[--bp_idx]) {
                            --rw_idx;
        ...
        children(const node_type& v) const
        {
            ...
            if (!is_leaf(v)) {
                ...
                auto x = std::get<0>(v.p);
                auto y = std::get<1>(v.p);
                auto z = std::get<2>(v.p);
                ...
                for (size_t i=0; i<t_k; ++i) {
                    for (size_t j=0; j<t_k; ++j) {
                        for (size_t k=0; k<t_k; ++k) {
                            ...
                            if (m_bp[ v.idx+t_k*t_k*i+t_k*j+k  ]) {
                                ++rank;

                                auto _x = x + i*precomp<t_k>::exp(v.t-1);
                                auto _y = y + j*precomp<t_k>::exp(v.t-1);
                                auto _z = z + k*precomp<t_k>::exp(v.t-1);

                                auto _max_v = v.max_v - m_maxval[rank];
                                auto _max_p = t_p {{_x, _y, _z}};
                                if (v.t > 1) {
                                    auto ci = rank-m_level_idx[v.t-1];
                                    _max_p = t_p {{
                                            _x+m_coord[v.t-2][3*ci],
                                            _y+m_coord[v.t-2][3*ci+1],
                                            _z+m_coord[v.t-2][3*ci+2]
                                        }
                                    };
                                }
                                res.emplace_back(v.t-1, t_p {{_x,_y,_z}}, rank*t_k*t_k*t_k,
                                _max_v, _max_p);
                            }
                        }
                    }
    ...
    std::pair<k3_treap_ns::point_type, k3_treap_ns::point_type>
    bounding_box(const node_type& v) const {
        if (is_leaf(v))
            return {v.max_p, v.max_p};
        uint64_t size = (uint64_t{1} << v.t) - 1;
        return {v.p, {v.p[0] + size, v.p[1] + size, v.p[2] + size}};
    }
};
